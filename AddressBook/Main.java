		...
		do {
			...
			case 1:
				...
				if (yesOrNo.toLowerCase().equals("y")) {
					...
					while (customFieldBoolean == false) {
						...
						System.out.print("Would you like to add another custom field (Y/N): ");
						yesOrNo = in.nextLine();
			...
			case 3:
				...
				if (crudeUpdateInfo.contains(","))
					...
					System.out.println("Info was not entered correctly, please make sure to use a comma!");
			...
			case 5:
				...
				if (matches.size() == 0) {
					...
					System.out.println("There were " + matches.size()+ " results found.");
				System.out.println("Would you like to print out the results (Y/N)?");
				yesOrNo = "n";
				yesOrNo = in.nextLine();
				if (yesOrNo.toLowerCase().equals("y")) {
